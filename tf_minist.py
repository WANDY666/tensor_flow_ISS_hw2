...
def train_net(net, batch_size, epoch, train_db, val_db, summary_writer):
    ...
    for i in range(epoch):
        ...
        for offset in range(0, train_samples, batch_size):
            ...
            batch_train_images, batch_train_labels = train_images[offset:end], train_labels[
            ...
            _, loss, loss_summary = sess.run([training_operation, loss_operation, merge_summary],
                                             feed_dict={input: batch_train_images, labels: batch_train_labels})
            total_loss += loss
    ...
    if os.path.exists('./model') == False:
        os.makedirs('./model')
    output_graph_def = convert_variables_to_constants(sess, sess.graph_def, output_node_names=['output', 'loss',
                                                                                               'accuracy'])  # set saving node
